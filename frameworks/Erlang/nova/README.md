...
You can create multiple implementations and they will all conform to `[name in benchmark_config.json].dockerfile`. For example, the `default` implementation in `benchmark_config.json` will be `nova.dockerfile`, but if you wanted to make another implementation that did only the database tests for MySQL, you could make `nova-mysql.dockerfile` and have an entry in your `benchmark_config.json` for `nova-mysql`.

4. Test your application

        $ tfb --mode verify --test nova
...
Edit `.github/workflows/build.yml` to ensure that Github Actions will automatically run our verification tests against your new test. This file is kept in alphabetical order, so find where `TESTDIR=Erlang/nova` should be inserted under `env > matrix` and put it there.
